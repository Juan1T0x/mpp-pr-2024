...
for file in result_files:
    try:
        # Extraer n, m, n_gen, tam_pob, c_convergencia del nombre del archivo
        parts = file.split('_')
        n = int(parts[1])
        m = int(parts[2])
        n_gen = int(parts[3])
        tam_pob = int(parts[4])
        c_convergencia = float(parts[5].split('.')[0] + '.' + parts[5].split('.')[1])  # Asegura capturar correctamente el valor decimal
...
df = df[["n", "m", "n_gen", "tam_pob", "fitness", "exe_time", "c_convergencia"]]
