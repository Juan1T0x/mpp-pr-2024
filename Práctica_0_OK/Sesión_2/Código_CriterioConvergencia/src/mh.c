 ...
 * @param n_gen Número máximo de generaciones a simular.
 * @param tam_pob Tamaño de la población.
 * @param c_convergencia Umbral de convergencia para el fitness.
...
{
	// Reinicia la secuencia pseudoaleatoria en cada ejecución.
	srand(time(NULL) + getpid());
	int i, g, mutation_start;

	// Crea la población inicial (array de punteros a individuos).
	Individuo **poblacion = (Individuo **)malloc(tam_pob * sizeof(Individuo *));
	assert(poblacion);

	// Genera cada individuo de la población inicial.
	for (i = 0; i < tam_pob; i++)
	{
	...
	// Ordena la población según el fitness (de mayor a menor).
	qsort(poblacion, tam_pob, sizeof(Individuo *), comp_fitness);

	// Variables para control de convergencia.
	double best_fitness = poblacion[0]->fitness; // Mejor fitness actual.
	double prev_fitness = best_fitness;			 // Fitness de la generación anterior.
	int gen_without_improvement = 0;			 // Contador de generaciones sin mejora.

	// Evoluciona la población durante un máximo de 'n_gen' generaciones o hasta convergencia.
	for (g = 0; g < n_gen; g++)
	{
		// Los hijos de los padres más aptos reemplazan a la mitad menos apta de la población.
		for (i = 0; i < (tam_pob / 2) - 1; i += 2)
		{
			// Asegura que los hijos tienen memoria asignada para sus arrays.
			...
			// Realiza el cruce entre pares de padres para generar nuevos hijos.
			cruzar(poblacion[i], poblacion[i + 1], poblacion[tam_pob / 2 + i], poblacion[tam_pob / 2 + i + 1], n, m);
		}

		// Determina a partir de qué posición se aplicará la mutación (desde 1/4 de la población).
		mutation_start = tam_pob / 4;

		// Aplica mutación al 75% de la población.
		...
		// Vuelve a ordenar la población según el fitness.
		qsort(poblacion, tam_pob, sizeof(Individuo *), comp_fitness);

		// Obtén el mejor fitness de esta generación.
		best_fitness = poblacion[0]->fitness;

		// Verifica si hay mejora en el fitness respecto a la generación anterior.
		if (fabs(best_fitness - prev_fitness) < c_convergencia)
		{
			gen_without_improvement++;
		}
		else
		{
			gen_without_improvement = 0; // Reinicia el contador si hay mejora.
		}

		// Actualiza el fitness de la generación anterior.
		prev_fitness = best_fitness;

		// Imprime información si está habilitado.
		if (PRINT)
		{
		...
		// Si no hay mejora después de varias generaciones, termina la ejecución.
		if (gen_without_improvement > 10)
		{
			if (PRINT)
				printf("Convergencia alcanzada en la generación %d\n", g);
			break;
		}
	}

	// Ordena el array de la mejor solución encontrada.
	qsort(poblacion[0]->array_int, m, sizeof(int), comp_array_int);

	// Copia la mejor solución al array 'sol' proporcionado.
	memmove(sol, poblacion[0]->array_int, m * sizeof(int));

	// Almacena el valor de fitness de la mejor solución.
	double value = poblacion[0]->fitness;

	// Libera la memoria asignada a la población.
	for (i = 0; i < tam_pob; i++)
	{
	...
	// Devuelve el valor de fitness de la mejor solución.
	return value;
}
