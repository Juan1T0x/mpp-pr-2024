...
for file in result_files:
    try:
        # Extraer n, m, n_gen, tam_pob, nthreads del nombre del archivo
        parts = file.split('_')
        n = int(parts[1])
        m = int(parts[2])
        n_gen = int(parts[3])
        tam_pob = int(parts[4])
        nthreads = int(parts[5].split('.')[0])  # Eliminar la extensión del último valor
...
df = df[["n", "m", "n_gen", "tam_pob", "fitness", "exe_time", "nthreads"]]
