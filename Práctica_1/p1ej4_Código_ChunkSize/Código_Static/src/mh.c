...
#pragma omp parallel private(i)
	{
		...
		// Generación de la población inicial con scheduling estático y chunk_size
#pragma omp for schedule(static, chunk_size)
	...
	for (g = 0; g < n_gen; g++)
	{
		// Paralelizamos el cruce de padres, ya que cada cruce es independiente
#pragma omp parallel private(i)
		{
#pragma omp for schedule(static, chunk_size)
...
// Aplica mutación con schedule(static, chunk_size)
...
// Recalcula el fitness con schedule(static, chunk_size)
#pragma omp parallel for private(i) schedule(static, chunk_size)
		for (i = 0; i < tam_pob; i++)
		{
			fitness(d, poblacion[i], n, m);
